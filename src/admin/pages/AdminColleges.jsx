...
import { adminGetColleges, adminUpsertCollege, adminDeleteCollege, adminToggleFeatured } from "../../services/admin"
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-[11px] text-on-surface-variant mt-1">{colleges.length} colleges</p>
                </div>
            ...
            {loading && colleges.length === 0 ? (
                ...
                <div className="rounded-2xl border border-outline-variant/10 bg-[#0d0e1a] overflow-hidden">
                    <table className="w-full">
                        ...
                        <tbody>
                            {colleges.map((c, i) => (
                                <tr key={c.id} className={`border-b border-white/[0.02] ${i % 2 === 0 ? "glass/[0.01]" : ""} hover:glass/[0.03] transition`}>
                                    <td className="px-5 py-3 text-xs font-semibold text-white">{c.name}</td>
                                    <td className="px-5 py-3 text-xs text-on-surface-variant">{c.city || "—"}</td>
                                    <td className="px-5 py-3 text-xs text-on-surface-variant">{c.state || "—"}</td>
                                    <td className="px-5 py-3"><span className="px-2 py-0.5 rounded-full text-[8px] font-black bg-primary/100/10 text-primary/70 border border-primary/15">{c.admission_mode || "—"}</span></td>
                                    <td className="px-5 py-3">
                                        <button onClick={() => handleToggleFeatured(c.id, !c.is_featured)}
                                            className={`p-1 rounded-lg transition ${c.is_featured ? "text-tertiary hover:text-amber-300" : "text-on-surface hover:text-tertiary"}`}>
                                            <Star size={14} fill={c.is_featured ? "currentColor" : "none"} />
                                        </button>
                                    </td>
                                    <td className="px-5 py-3"><div className="flex gap-1">
                                        <button onClick={() => startEdit(c)} className="p-1.5 rounded-lg hover:bg-primary/100/10 text-on-surface-variant hover:text-primary/70"><Pencil size={12} /></button>
                                        <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg hover:bg-error/100/10 text-on-surface-variant hover:text-error"><Trash2 size={12} /></button>
                                    </div></td>
                                </tr>
                            ))}
                            {colleges.length === 0 && <tr><td colSpan={6} className="px-5 py-12 text-center text-xs text-on-surface-variant">No colleges yet</td></tr>}
                        </tbody>
                    </table>
                </div>
            )}
