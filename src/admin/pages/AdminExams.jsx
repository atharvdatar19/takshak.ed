...
import { adminGetExams, adminUpsertExam, adminDeleteExam } from "../../services/admin"
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-[11px] text-on-surface-variant mt-1">{exams.length} exams</p>
                </div>
                        ...
                        {[["Exam Name", "name", "text"], ["Stream", "stream", "text"], ["Start Date", "start_date", "date"], ["End Date", "end_date", "date"]].map(([label, key, type]) => (
                            <div key={key}>
                                ...
                                <input type={type} value={form[key] || ""} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                                    className="w-full rounded-xl border border-white/[0.05] glass/[0.02] px-3 py-2 text-xs text-on-surface-variant/40 outline-none focus:border-primary/40 transition" />
                            </div>
                        ))}
                        <div>
                            <label className="text-[9px] text-on-surface-variant font-black uppercase tracking-wider block mb-1">Priority</label>
            ...
            {loading && exams.length === 0 ? (
                ...
                <div className="rounded-2xl border border-outline-variant/10 bg-[#0d0e1a] overflow-hidden">
                    <table className="w-full">
                        ...
                        <tbody>
                            {exams.map((e, i) => (
                                <tr key={e.id} className={`border-b border-white/[0.02] ${i % 2 === 0 ? "glass/[0.01]" : ""} hover:glass/[0.03] transition`}>
                                    <td className="px-5 py-3 text-xs font-semibold text-white">{e.name}</td>
                                    <td className="px-5 py-3 text-xs text-on-surface-variant">{e.stream || "—"}</td>
                                    <td className="px-5 py-3 text-xs text-on-surface-variant">{e.start_date || "—"}</td>
                                    <td className="px-5 py-3 text-xs text-on-surface-variant">{e.end_date || "—"}</td>
                                    <td className="px-5 py-3 text-xs text-tertiary font-bold">P{e.priority || 0}</td>
                                    <td className="px-5 py-3">{e.is_national && <span className="text-tertiary text-[10px]">✓</span>}</td>
                                    <td className="px-5 py-3"><div className="flex gap-1">
                                        <button onClick={() => startEdit(e)} className="p-1.5 rounded-lg hover:bg-primary/100/10 text-on-surface-variant hover:text-primary/70"><Pencil size={12} /></button>
                                        <button onClick={() => handleDelete(e.id)} className="p-1.5 rounded-lg hover:bg-error/100/10 text-on-surface-variant hover:text-error"><Trash2 size={12} /></button>
                                    </div></td>
                                </tr>
                            ))}
                            {exams.length === 0 && <tr><td colSpan={7} className="px-5 py-12 text-center text-xs text-on-surface-variant">No exams yet</td></tr>}
                        </tbody>
                    </table>
                </div>
            )}
