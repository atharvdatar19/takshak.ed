...
export async function getColleges(params = {}) {
  ...
  if (status === "open") query = query.gte("application_end", nowIso)
  if (status === "closed") query = query.lt("application_end", nowIso)
  if (status === "closingSoon") {
    const soon = new Date()
    soon.setDate(soon.getDate() + 5)
    query = query.gte("application_end", nowIso).lte("application_end", soon.toISOString().slice(0, 10))
  }

  const from = (page - 1) * pageSize
  const to = from + pageSize - 1

  const { data, error, count } = await query.order("is_featured", { ascending: false }).order("application_end", { ascending: true }).range(from, to)
...
export async function getExamsTimeline(params = {}) {
  ...
  let query = supabase.from("exams_timeline").select("*").order("priority", { ascending: false }).order("start_date", { ascending: true })
