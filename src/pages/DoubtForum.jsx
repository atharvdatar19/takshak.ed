        ...
        <div className="space-y-10 md:space-y-16">
            ...
            <AnimatePresence>
                {showPost && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                        <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="mx-4 w-full max-w-lg space-y-5 rounded-[28px] glass p-8 shadow-2xl">
                            <div className="flex items-center justify-between">
                                <h3 className="text-section text-xl text-on-surface">Ask Anonymously</h3>
                                <button type="button" onClick={() => setShowPost(false)} className="text-on-surface-variant/60 hover:text-on-surface-variant"><X size={20} /></button>
                            </div>
                            <select value={newPost.stream} onChange={e => setNewPost(p => ({ ...p, stream: e.target.value }))} className="w-full rounded-2xl border border-outline-variant/20 bg-surface-container-low px-4 py-3 text-sm">
                                {STREAMS.filter(s => s !== "All").map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                            <input value={newPost.title} onChange={e => setNewPost(p => ({ ...p, title: e.target.value }))} placeholder="Your question (clear & specific)" className="w-full rounded-2xl border border-outline-variant/20 bg-surface-container-low px-4 py-3 text-sm outline-none focus:border-primary/40" />
                            <textarea value={newPost.body} onChange={e => setNewPost(p => ({ ...p, body: e.target.value }))} placeholder="Add details..." rows={4} className="w-full rounded-2xl border border-outline-variant/20 bg-surface-container-low px-4 py-3 text-sm outline-none" />
                            <p className="text-xs text-on-surface-variant/60">🔒 Your identity is never shared</p>
            ...
            <div className="space-y-5">
                {filtered.map((post, i) => {
                    ...
                    return (
                        <div key={post.id} className={`reveal reveal-delay-${(i % 4) + 1} glass rounded-lg overflow-hidden`}>
                            <div className="p-6 md:p-8">
                                <div className="flex items-start gap-4">
                                    <div className="flex flex-col items-center gap-1">
                                        <button type="button" onClick={() => toggleUpvote(post.id, "post")} className={`rounded-xl p-2 transition ${upvoted[upvoteKey] ? "bg-primary/15 text-primary" : "text-on-surface-variant/60 hover:text-primary"}`}>
                                            <ThumbsUp size={16} />
                                        </button>
                                        <span className="text-sm font-bold text-on-surface">{post.upvotes + (upvoted[upvoteKey] ? 1 : 0)}</span>
                                    </div>
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="pill pill-outline text-[10px] py-0.5 px-2.5">{post.stream}</span>
                                            <span className="text-xs text-on-surface-variant/60">{new Date(post.created_at).toLocaleDateString("en-IN")}</span>
                                        </div>
                                        <h3 className="text-card-title text-base md:text-lg text-on-surface">{post.title}</h3>
                                        <p className="mt-2 text-sm text-on-surface-variant leading-relaxed">{post.body}</p>
                                        <button type="button" onClick={() => setExpandedPost(isExpanded ? null : post.id)} className="mt-3 flex items-center gap-1.5 text-sm font-semibold text-primary hover:text-primary">
                            ...
                            <AnimatePresence>
                                {isExpanded && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-outline-variant/10 bg-surface-container-low/50 px-6 py-5 md:px-8 space-y-3">
                                        {replies.map(r => (
                                            <div key={r.id} className={`rounded-2xl p-4 text-sm ${r.is_mentor ? "glass rounded-lg border-primary/20 bg-primary/10" : "glass rounded-lg"}`}>
                                                <p className="mb-1 text-xs font-bold text-primary">{r.is_mentor ? "✨ Mentor" : "Anonymous"}</p>
                                                <p className="text-on-surface leading-relaxed">{r.body}</p>
                                            </div>
                                        ))}
                                        <div className="flex gap-2">
                                            <input value={newReply} onChange={e => setNewReply(e.target.value)} placeholder="Add a reply..." className="flex-1 rounded-2xl border border-outline-variant/20 glass px-4 py-3 text-sm outline-none focus:border-primary/40" />
